...
// 🆕 통신 안정성 개선 변수들
uint8_t flash_save_pending = 0;      // Flash 저장 대기 플래그
uint8_t update_flow_pending = 0;     // Flow 업데이트 대기 플래그
uint32_t last_update_time = 0;       // 마지막 업데이트 시간
uint16_t communication_error_cnt = 0; // 통신 에러 카운터
uint8_t modbus_connection_status = 1; // Modbus 연결 상태

// 🆕 디버깅 및 진단 변수들
uint16_t DebugStep = 0;              // 디버깅 단계
uint16_t DebugAddr = 0;              // 디버깅 주소
uint16_t DebugValue = 0;             // 디버깅 값
uint16_t PasswordChangeTracker = 0;   // 패스워드 변경 추적
uint16_t PasswordChangeLocation = 0;  // 패스워드 변경 위치
...
uint8_t GasName[2*6];       // 동적으로 변경 가능하도록 수정
uint8_t GasUnit[2*3];       // 동적으로 변경 가능하도록 수정
...
// 🆕 고급 기능 함수 프로토타입들
//------------------------------------------------------------------------------------------------------
void UpdateFlowLampColors();         // 실시간 흐름 표시 업데이트
void ProcessAutoSwitch();            // 자동 전환 로직
void ProcessAlarmSystem();           // 알람 시스템 처리
void ProcessSimulationMode();        // 시뮬레이션 모드 처리
void VerifyPassword();               // 패스워드 검증
void SaveFlashParam();               // Flash 파라미터 저장
void ProcessBackgroundTasks();       // 백그라운드 작업 처리
void CheckCommunicationHealth();     // 통신 상태 체크
...
void SaveFlashParam()
{
	...
	fParam.PT1Data[0] = PT1.Low;
	fParam.PT1Data[1] = PT1.LowLow;
	fParam.PT1Data[2] = PT1.Offset;
	fParam.PT1Data[3] = PT1.MinSValue;
	fParam.PT1Data[4] = PT1.MaxSValue;
	fParam.PT1Data[5] = PT1.MaxValue;
...
// 🆕 시뮬레이션 모드 처리
//------------------------------------------------------------------------------------------------------
void ProcessSimulationMode()
{
	if(!simulation_mode) return;

	static uint8_t sim_counter = 0;
	if(++sim_counter < 10) return;  // 10번에 1번만 실행 (속도 조절)
	sim_counter = 0;
...
{
	if(!auto_switch_enabled || IOdata.SupplyGasLSW != 1) return;
...
void ProcessAlarmSystem()
{
	...
	if(PT3.Value < PT3.MinSValue) {
		if(!PT3_low_acknowledged) {
			IOdata.MinSuuplPressPT3 = 1;  // PT3 최소공급알람 ON
		}
	} else {
		IOdata.MinSuuplPressPT3 = 0;
		PT3_low_acknowledged = 0;
	}
...
void UpdateFlowLampColors()
{
	...
	// 왼쪽 가스(PT1) 공급 중일 때 색상 업데이트
	if(IOdata.SupplyGasLSW == 1) {
		if (PT1.Value > PT1.Low) {
			FlowLamp.BSVelL = 8;   // 정상 (녹색)
			FlowLamp.BSLampL = 8;
			FlowLamp.ASVelL = 8;
			FlowLamp.ASLampL = 8;
			// 알람 해제
...
void CheckCommunicationHealth()
{
	...
	// 5초 이상 통신이 없으면 연결 불량으로 판정
	if(current_time - last_comm_time > 5000) {
		modbus_connection_status = 0;
		communication_error_cnt++;
	} else {
...
//------------------------------------------------------------------------------------------------------
// 🆕 백그라운드 작업 처리 (메인 루프에서 호출)
//------------------------------------------------------------------------------------------------------
void ProcessBackgroundTasks()
{
	static uint32_t last_flow_update = 0;
	uint32_t current_time = HAL_GetTick();

	// 1. Flash 저장 처리 (인터럽트 외부에서)
	...
	// 2. Flow 업데이트 처리 (100ms마다)
	if(update_flow_pending && (current_time - last_flow_update > 100)) {
		UpdateFlowLampColors();
		update_flow_pending = 0;
		last_flow_update = current_time;
	}
...
//	FLASH_USER_END_ADDR =0x0803FFFF;  
	...
	// 🆕 시뮬레이션 모드일 때 최댓값으로 초기화
	if(simulation_mode) {
		PT1.Value = 1000;  // PT1 설정값으로 시작
		PT2.Value = 1000;  // PT2 설정값으로 시작
		PT3.Value = 100; // PT3 설정값으로 시작
	} else {
...
//				else if(NewAddr == 2) 	SetConfigLamp();	// 설정화면으로
				else if(NewAddr == 2) 	OpenWinInputPW();	// 🆕 설정화면은 패스워드 입력으로
				...
				else if(NewAddr == 80) 	SetSolNONC();		// SOL NO/NC
				else if(NewAddr == 81) 	ToggleUseBuzzer();	// 부저사용/미사용
//				else if(NewAddr == 89) 	InputPassword(NewData);	// 비밀번호 입력
				else if(NewAddr == 89) 	VerifyPassword();	// 🆕 패스워드 검증
				...
				else if(NewAddr == 93) 	SetMainLamp();		// X 버튼 - 메인 창으로
...
//------------------------------------------------------- // Write Multiple Registers : WORD
				...
				if(NewByteCount==2) {
					...
					DebugAddr = NewAddr;
					DebugValue = NewData;
					// 🆕 패스워드 입력창에서 패스워드 입력 시
//					if(NewWindow == PW_Input && NewAddr == 90) {
					...
					// 🆕 패스워드 변경
						...
						flash_save_pending = 1;  // 🆕 Flash 저장을 나중에 처리
						DebugStep = 5000;
					}
...
//------------------------------------------------------- // End of Modbus Send Reply:			
    ...
    // 🔥 Flow 업데이트 주기 감소 (성능 개선)
    static uint8_t flow_counter = 0;
    if(++flow_counter >= 50) { // 50번에 1번만 실행 (부하 대폭 감소)
        flow_counter = 0;
        update_flow_pending = 1; // 기존 변수 활용
    }
}
...
void OnChangePW()
{
	// 🆕 ChangedValue가 실제로 변경되었는지 확인
	if(Password.ChangedValue != 0) {
		Password.CurValue = Password.ChangedValue;
		Password.SavedValue = Password.ChangedValue;
		flash_save_pending = 1;  // Flash 저장을 나중에 처리
	}
...
{
	DebugStep = 7777;
	DebugAddr = Password.CurValue;
	DebugValue = fParam.Password;
	...
	if(Password.CurValue == Password.SavedValue) {
		SetConfigLamp();  // 설정 화면으로
		PasswordChangeLocation = 3100;  // 성공
	} else {
		IOdata.ConfirmPWSW = 0;
		NewWindow = PW_Wrong;  // 패스워드 틀림 화면
