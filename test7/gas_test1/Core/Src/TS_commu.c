...
// TS_commu.c의 check_uart_status() 함수를 다음과 같이 수정
void check_uart_status(void)
{
    static uint16_t test_counter = 0;
    static uint16_t prev_RxPtr3 = 0;

    test_counter++;

    // 🔥 강제 화면 전환 테스트 (10초마다 자동으로 화면 변경)
    if(test_counter == 5000) {        // 약 10초 후
        SetConfigLamp();               // 설정화면으로 강제 변경
        DebugStep = 9001;
        DebugAddr = NewWindow;
        DebugValue = 41;
    }
    else if(test_counter == 10000) {   // 약 20초 후
        SetAlarmLamp();                // 알람화면으로 강제 변경
        DebugStep = 9002;
        DebugAddr = NewWindow;
        DebugValue = 45;
    }
    else if(test_counter == 15000) {   // 약 30초 후
        SetMainLamp();                 // 메인화면으로 강제 변경
        DebugStep = 9003;
        DebugAddr = NewWindow;
        DebugValue = 11;
        test_counter = 0;              // 리셋해서 반복
    }

    // UART 수신 확인
    if(RxPtr3 != prev_RxPtr3) {
        DebugStep = 1000;
        DebugAddr = RxPtr3;
        DebugValue = RxBuf3[prev_RxPtr3];
        prev_RxPtr3 = RxPtr3;
    }

    // Modbus 명령 확인
    if(NewFunc != 0) {
        DebugStep = 2000 + NewFunc;
        DebugAddr = NewAddr;
        DebugValue = NewData;
    }

    // 현재 화면 상태 항상 표시
    if(test_counter % 1000 == 0) {  // 1초마다
        DebugStep = 8000 + NewWindow;  // 8011=메인, 8041=설정, 8045=알람
        DebugAddr = test_counter / 1000;  // 경과 시간(초)
        DebugValue = NewWindow;
    }
}
...
////				RxCRC1[CmdIdx] = calculateCRC(&Data[CmdIdx][0], DataIdx-2);
				...
				// 🔥 CRC 체크 결과 디버깅 추가
				DebugStep = 6000;                    // CRC 체크 중
				DebugAddr = RxCRC[CmdIdx];          // 수신된 CRC
				DebugValue = RxCRC2[CmdIdx];        // 계산된 CRC

				// CRC가 일치하는지 확인
				if(RxCRC[CmdIdx] == RxCRC2[CmdIdx]) {
				    DebugStep = 7000;               // CRC 일치, 명령 처리 진행
				    PrevIdx = CmdIdx;
				    CmdIdx = (CmdIdx+1) & 7;
				} else {
				    DebugStep = 8000;               // CRC 불일치, 명령 무시
				    NewFunc = 0;                    // 명령 처리하지 않음
				    // 디버깅을 위해 NoRespCnt 증가
				    NoRespCnt++;
				    NewFunc0 = NewFunc;
				    NewAddr0 = NewAddr;
				}
...
/*
				...
				else if(NewByteCount == 12) {  // ← 이 조건 추가
					// 가스명 변경 (주소 0-5)
