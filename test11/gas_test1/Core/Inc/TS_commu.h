...
typedef enum { Main = 11, Config = 41, Alarm = 45, PW_Input = 111, PW_Change = 112, PW_Wrong = 113, PW_Change_OK = 114, AlertAlarm = 120 } Window_t;
...
typedef struct {
	...
	uint16_t Password;
	uint16_t PT1Data[6];	
	// [0]Low-GasAlarmReplace, [1]LowLow-GasPrepReplace, [2]Offset, 
	// [3]MinSValue / [4]MaxSValue - Min./Max. Value of Sensor, [5]MaxValue-Max. Value of Gas Chaeging
	uint16_t PT2Data[6];
	uint16_t PT3Data[4];
	// [0]MinSValue-Min. Supply Value. [1]Reserved, [2]Offset, [3]MaxSValue- Max. Value of Sensor
	uint32_t dummy;
} FlashParam_t;
...
// 👇 여기에 추가
void SaveFlashParam(void);  // 새로 추가할 함수 선언
void ReadFlashParam(void);

// ⭐⭐⭐ 최적화 관련 새로 추가된 함수 선언 ⭐⭐⭐
void ProcessBackgroundTasks(void);  // 메인 루프에서 호출할 백그라운드 작업 처리 함수
void UpdateFlowLampColors(void);    // Flow 램프 색상 업데이트 (무거운 작업)
void UpdateGraphSettings(void);      // 함수 선언 추가
void UpdateGasLampStatus(void);      // 함수 선언 추가
void ProcessADCValues(uint16_t adc1, uint16_t adc2, uint16_t adc3);
void UpdateSlaveAddress(uint8_t new_address);
// ===== 기존 TS_commu.h 파일에서 함수 선언 부분에 이것들만 추가하세요 =====
...
// 전역 변수들 (기존 extern 선언들 근처에 추가)
extern uint8_t emergency_stop_active;
extern uint8_t pressure_sensor_active;
extern uint8_t prev_emergency_state;
extern uint16_t DebugStep;
